...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilter(HttpSecurity http) throws Exception {
		...
		//모든페이지에 대해서 거부
		//http.authorizeHttpRequests(autorize -> autorize.anyRequest().denyAll());
...
//												authenticated());
		//user페이지에 대해서 권한이 필요
//		http.authorizeHttpRequests(autorize -> autorize.antMatchers("/user/**").hasRole("USER")
//													   .antMatchers("/admin/**").hasRole("USER"));
		
		//all페이지는 인증만 되면 됨, user페이지는 user권한, admin페이지는 admin권한, 나머지 모든 페이지는 접근이 가능함.
//		http.authorizeHttpRequests(autorize -> autorize.antMatchers("/all").authenticated()
...
//				                                       .anyRequest().permitAll());		
		...
		http.rememberMe()
		    ...
		    .userDetailsService(myUserDetailService) //토근이 있을때 실행시킬 userDetailService객체(필수)
